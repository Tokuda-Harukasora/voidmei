...
public class Controller {
	...
	private void registerGameModeOverlays() {
		...
		overlayManager.registerWithPreview("crosshairSwitch",
				...
				.withInterest("displayCrosshair", "drawHUD", "disableHUD", "crosshair", "miniHUD", "enableLayoutDebug",
						...
						"alwaysShowRadarAltitude", "modernHudStyle");
	...
	public synchronized Blkx getBlkx() {
		...
		if ((Blkx == null || !Blkx.valid) && State == ControllerState.PREVIEW) {
			...
			if (fallbackPlane != null && !fallbackPlane.isEmpty()
				...
				// 修复: 回退成功后更新 identifiedFMName，防止重复回退循环
				if (Blkx != null && Blkx.valid) {
					identifiedFMName = fallbackPlane;
				}
			}
		}
	...
	public void loadFMData(String planename) {
		...
		// 修复: 即使FM加载失败也要设置loadedFMName，防止无限循环重试
		loadedFMName = planename;
		// 成功加载时同步 identifiedFMName（但不改 cur_fmtype，避免 Service 检测到误变更）
		if (Blkx != null && Blkx.valid) {
			identifiedFMName = planename;
		}
	}
